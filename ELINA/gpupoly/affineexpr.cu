...
template<typename T>
void AffineExpr<T>::print()const{
	std::cout << "AffineExpr\n";
	if(this->A.get() && this->b.get()){
		std::cout << "A= "; this->A.get()->print();
	}else std::cout << "A= nullptr\n";
	if(this->b.get()){
		std::cout << "b= "; this->b.get()->print();
	}else std::cout << "b= nullptr\n";
	if(this->rows){
		std::cout << "rows= ";
		std::vector<int> rows(this->m);
		cudaMemcpy(rows.data(),this->rows,this->m*sizeof(int),cudaMemcpyDeviceToHost);
		for(size_t i=0; i < this->m; i++){
			std::cout << rows[i] << " ";
		}
		std::cout << std::endl;
	}else std::cout << "rows= nullptr\n";
