...
//! Prints the content of the Matrix.
	void print() const {
		if (interval_)
		{
			...
			std::cout << "n_ = " << n_ << " m_ = " << m_ << " N=" << N_ << std::endl;
			for (int i = 0; i < m_; i++)
			{
				for (int j = 0; j < n_; j++)
					std::cout << "["<< tmp[i*n_+j].low << " " << tmp[i * n_ + j].high << " at " << i * n_ + j << "] ";
				std::cout << std::endl;
			}
			std::cout << std::endl;
		}
...
*/
	...
	void modify_single(size_t rowNum, size_t colNum, T value);
	
	void modify_column(size_t colNum, T value);

	void modify_single_range(size_t rowNum, size_t colNum, T lb, T ub);
