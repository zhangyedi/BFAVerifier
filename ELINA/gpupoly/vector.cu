...
template<typename T>
__global__ void IntervalUnionKernel(Intv<T>* dest, const Intv<T>* lhs, const Intv<T>* rhs, size_t n)
{
	size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx < n){
		dest[idx].low = fmin(lhs[idx].low, rhs[idx].low);
		dest[idx].high = fmax(lhs[idx].high, rhs[idx].high);
	}
}
 
template <typename T>
int Vector<T>::unionIntervals(Vector<T>&dest, const Vector<T>&lhs, const Vector<T>&rhs){
	if (lhs.size() != rhs.size() || !lhs.interval() || !rhs.interval() || !dest.interval()){
		std::cerr << "Error: unionIntervals: incompatible sizes or interval types" << std::endl;
		return 1;
	}
	IntervalUnionKernel<T> << <(dest.size() + 255) / 256, 256 >> > (dest, lhs, rhs, dest.size());
	return 0;
}
