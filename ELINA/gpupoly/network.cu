...
template <>
std::map<std::string, std::vector<std::shared_ptr<Vector<float>>>>&
NeuralNetwork::getRecordConcreteBoundsMap()
{
	return recordConcreteBoundsS_Map;
}

template <>
std::map<std::string, std::vector<std::shared_ptr<Vector<double>>>>&
NeuralNetwork::getRecordConcreteBoundsMap()
{
	return recordConcreteBoundsD_Map;
}

template <>
std::shared_ptr<Vector<float>> NeuralNetwork::getRecordConcreteBounds(int layer)
{
	return recordConcreteBoundsS[layer];
}

template <>
std::shared_ptr<Vector<double>> NeuralNetwork::getRecordConcreteBounds(int layer)
{
	return recordConcreteBoundsD[layer];
}
...
{
	// std::cout<<"===evaluateAffine Begin on " << layer << "=====\n";
	// if (A)
	// {
	// 	std::cout<<"A\n";
	// 	A->print();
	...
	// dest.print();
	// std::cout<<"===evaluateAffine end of " << layer << "=====\n";
}
...
int NeuralNetwork::addLayer(Layer* layer)
{
	...
	recordConcreteBoundsS.push_back(std::make_shared<Vector<float>>());
	recordConcreteBoundsD.push_back(std::make_shared<Vector<double>>());
...
bool NeuralNetwork::run(const Vector<T>& input, const int label, bool sound)
{
	...
	if (!this->enableIntegrateTestGetter() 
		&& this->enableRecordConcreteBoundsGetter() == RecordStatus::NO_RECORD
		&& res.isPositive())
		return true;
	// Vector<T> test;
	...
	// printf("res(2)="); res.print();
	bool resPos = res.isPositive();

	// return c;
	if (this->enableRecordConcreteBoundsGetter() != RecordStatus::NO_RECORD){
		if (this->enableRecordConcreteBoundsGetter() != RecordStatus::RECORD_AND_UNION_IF_SAFE || resPos){
			for (int p = 1; p < layers.size(); p++){
				int errstatus = recordConreteBounds<T>(p);
				assert (errstatus == 0);
			}
		}
	}
...
template<typename T>
int NeuralNetwork::recordConreteBounds(int layer){
	if (this->enableRecordConcreteBoundsGetter() == RecordStatus::NO_RECORD){
		printf("Warning: recordConcreteBounds is not enabled but you called recordConreteBounds\n");
		return 0;
	}
	if (this->enableRecordConcreteBoundsGetter() == RecordStatus::RECORD_AND_OVERRIDE){
		if (std::is_same<T, float>::value){
			*recordConcreteBoundsS[layer] = *concreteBoundsS[layer];
		}
		else{
			*recordConcreteBoundsD[layer] = *concreteBoundsD[layer];
		}
		return 0;
	}
	if (this->enableRecordConcreteBoundsGetter() == RecordStatus::RECORD_AND_UNION
		|| this->enableRecordConcreteBoundsGetter() == RecordStatus::RECORD_AND_UNION_IF_SAFE){
		if (std::is_same<T, float>::value){
			int res = Vector<float>::unionIntervals(*recordConcreteBoundsS[layer], 
							*recordConcreteBoundsS[layer],
							*concreteBoundsS[layer]);
			return res;
		}else{
			int res = Vector<double>::unionIntervals(*recordConcreteBoundsD[layer], 
							*recordConcreteBoundsD[layer],
							*concreteBoundsD[layer]);
			return res;
		}
	}
	printf("Error: unknown RecordStatus \n");
	return -1;
}

template<typename T>
void NeuralNetwork::printConcreteBounds(int layer, const char* prelude){
	printf("%s ", prelude);
	if (std::is_same<T, float>::value){
		concreteBoundsS[layer]->print();
	}
	else{
		concreteBoundsD[layer]->print();
	}
}

template<typename T>
void NeuralNetwork::printRecordedBounds(int layer, const char* prelude){
	printf("%s ", prelude);
	if (std::is_same<T, float>::value){
		recordConcreteBoundsS[layer]->print();
	}
	else{
		recordConcreteBoundsD[layer]->print();
	}
}

template<typename T>
void NeuralNetwork::saveBoundsToMap(const char* name){
	if (std::is_same<T, float>::value){
		if(recordConcreteBoundsS_Map.find(name) == recordConcreteBoundsS_Map.end()){
			recordConcreteBoundsS_Map[name] = std::vector<std::shared_ptr<Vector<float>>>();
			for (int i = 0; i < recordConcreteBoundsS.size(); i++){
				recordConcreteBoundsS_Map[name].push_back(std::make_shared<Vector<float>>(
					*recordConcreteBoundsS[i]
				));
			}
			return;
		}
		for(int i = 0; i < recordConcreteBoundsS.size(); i++){
			*recordConcreteBoundsS_Map[name][i] = *recordConcreteBoundsS[i];
		}
	}
	else{
		if(recordConcreteBoundsD_Map.find(name) == recordConcreteBoundsD_Map.end()){
			recordConcreteBoundsD_Map[name] = std::vector<std::shared_ptr<Vector<double>>>();
			for (int i = 0; i < recordConcreteBoundsD.size(); i++){
				recordConcreteBoundsD_Map[name].push_back(std::make_shared<Vector<double>>(
					*recordConcreteBoundsD[i]
				));
			}
			return;
		}
		for(int i = 0; i < recordConcreteBoundsD.size(); i++){
			*recordConcreteBoundsD_Map[name][i] = *recordConcreteBoundsD[i];
		}
	}
}

template<typename T>
int NeuralNetwork::unionBoundsToRecordedBounds(const char* name){
	auto thisMap = getRecordConcreteBoundsMap<T>();
	for(int i = 0; i < (int)layers.size(); ++i)
		Vector<T>::unionIntervals(*getRecordConcreteBounds<T>(i), 
			*getRecordConcreteBounds<T>(i), 
			*thisMap[name][i]);
	gpuChkKer();
	return 0;
}
...
template void NeuralNetwork::saveBoundsToMap<float>(const char* name);
template void NeuralNetwork::saveBoundsToMap<double>(const char* name);

template int NeuralNetwork::unionBoundsToRecordedBounds<float>(const char* name);
template int NeuralNetwork::unionBoundsToRecordedBounds<double>(const char* name);

template void NeuralNetwork::printConcreteBounds<float>(int layer, const char* prelude);
template void NeuralNetwork::printConcreteBounds<double>(int layer, const char* prelude);

template void NeuralNetwork::printRecordedBounds<float>(int layer, const char* prelude);
template void NeuralNetwork::printRecordedBounds<double>(int layer, const char* prelude);
