...
*/
	...
	*/
	...
	template <typename T>
	std::map<std::string, std::vector<std::shared_ptr<Vector<T>>>>&
	getRecordConcreteBoundsMap(); 

	template <typename T>
	std::shared_ptr<Vector<T>> getRecordConcreteBounds(int layer);
	...
	*/
	...
	void enableIntegrateTestSetter(int enable) {
		this->enableIntegrateTest = enable;
	}
	int enableIntegrateTestGetter() {
		return this->enableIntegrateTest;
	}
	enum class RecordStatus {
		NO_RECORD,
		RECORD_AND_OVERRIDE,
		RECORD_AND_UNION,
		RECORD_AND_UNION_IF_SAFE,
	};
	void enableRecordConcreteBoundsSetter(RecordStatus enable){
		this->enableRecordConcreteBounds = enable;
	}
	RecordStatus enableRecordConcreteBoundsGetter() {
		return this->enableRecordConcreteBounds;
	}

	template<typename T>
	int recordConreteBounds(int layer);

	template<typename T>
	void printConcreteBounds(int layer, const char* prelude);

	template<typename T>
	void printRecordedBounds(int layer, const char* prelude);

	template<typename T>
	void saveBoundsToMap(const char* name);

	template<typename T>
	int unionBoundsToRecordedBounds(const char* name);
...
private:
	...
	int enableIntegrateTest = 0; // 0: no test, 1: test, 2: test and print
	RecordStatus enableRecordConcreteBounds = RecordStatus::NO_RECORD; 
	std::vector<std::shared_ptr<Vector<double>>> recordConcreteBoundsD;
	std::vector<std::shared_ptr<Vector<float>>> recordConcreteBoundsS;
	std::map<std::string, std::vector<std::shared_ptr<Vector<double>>>> recordConcreteBoundsD_Map;
	std::map<std::string, std::vector<std::shared_ptr<Vector<float>>>> recordConcreteBoundsS_Map;
	
	std::string saveMapKey;
};
