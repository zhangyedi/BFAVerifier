...
template<> inline Vector<double>& Sigmoid::activCst<double>() { return activCstD; }
template<> inline Vector<double>& Sigmoid::activFac<double>() { return activFacD; }
template<> inline Vector<float>& Sigmoid::activCst<float>() { return activCstS; }
template<> inline Vector<float>& Sigmoid::activFac<float>() { return activFacS; }
template<> inline const Vector<double>& Sigmoid::activCst<double>()const { return activCstD; }
template<> inline const Vector<double>& Sigmoid::activFac<double>() const { return activFacD; }
template<> inline const Vector<float>& Sigmoid::activCst<float>() const { return activCstS; }
template<> inline const Vector<float>& Sigmoid::activFac<float>() const { return activFacS; }


void Sigmoid::eval(Vector<double>& dest, bool sound, bool precise) {eval<double>(dest, sound, precise);}
void Sigmoid::eval(Vector<float>& dest, bool sound, bool precise) {eval<float>(dest, sound, precise);}

void Sigmoid::backSubstitute(typename AffineExpr<double>::Queue& queue, const AffineExpr<double>& expr) const {backSubstitute<double>(queue, expr);}
void Sigmoid::backSubstitute(typename AffineExpr<float>::Queue& queue, const AffineExpr<float>& expr) const {backSubstitute<float>(queue, expr);}
...
template<typename T>
__device__ T sigmoid(T z) {
    return 1.0 / (1.0 + exp(-z));
}


template<typename T>
__device__ T d_sigmoid(T z) {
    T y = sigmoid(z);
    return y * (1.0 - y);
}
...
/**
 * sigmoid_abstract computes two linear bounds for sigmoid(wx) in the range:
 ...
 */
template<typename T>
__device__ void sigmoid_weighted_abstract(T wl, T wu,
                                 T xl, T xu,
                                 ...
                                 slope_point<T> *sp2)  // upper bound
{
    ...
    if (fabs(xl - xu) < 1e-10) {
        T x = xl;
        // Return (0, wl * sigmoid(x)) and (0, wu * sigmoid(x)).
        *sp1 = slope_point<T>{0, wl * sigmoid(x)};
        *sp2 = slope_point<T>{0, wu * sigmoid(x)};
        return;
    }
    ...
    T gl = sigmoid(xl);
    T gu = sigmoid(xu);
    T gl_prime = d_sigmoid(xl);
    T gu_prime = d_sigmoid(xu);
    ...
    // 2) xu <= 0
        ...
        // 2a) wl >= 0
        if (wl >= 0.0) {
            *sp1 = slope_point_repre(wl * kappa_prime, xl, wl * gl);
            *sp2 = slope_point_repre(wu * kappa,       xu, wu * gu);
            return;
        }
        // 2b) wu <= 0
        else if (wu <= 0.0) {
            *sp1 = slope_point_repre(wl * kappa,       xu, wl * gu);
            *sp2 = slope_point_repre(wu * kappa_prime, xl, wu * gl);
            return;
        }
    }
    // 3) xl < 0 < xu
 ...
 */
template<typename T>
__device__ void sigmoid_abstract(T xl, T xu,
                                 slope_point<T> *sp1,  // lower bound
                                 slope_point<T> *sp2)  // upper bound
{
    ...
    if (fabs(xl - xu) < 1e-15) {
        T x = xl;
        // Return (0, wl * sigmoid(x)) and (0, wu * sigmoid(x)).
        *sp1 = slope_point<T>{0, sigmoid(x)};
        *sp2 = slope_point<T>{0, sigmoid(x)};
        return;
    }
...
template<typename T>
__global__ void evalSigmoidKernel(Intv<T>* dest, Intv<T>* activCst, Intv<T>* activFac, const Intv<T>* inputs, const size_t size) {
        size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < size) {
                Intv<T> input = inputs[idx];
                slope_point<T> lb_line, ub_line; 
                sigmoid_abstract(input.low, input.high, &lb_line, &ub_line);
                activFac[idx] = Intv<T>(lb_line.k, ub_line.k);
                activCst[idx] = Intv<T>(lb_line.b, ub_line.b);
                dest[idx] = Intv<T>(sigmoid(input.low), sigmoid(input.high));
        }
}

template<typename T>
__global__ void applyInfoSigmoid(Intv<T>* dest, Intv<T>* activCst, Intv<T>* activFac, const Intv<T>* inputs, const DPolyRInfo* infoPool, const size_t infoSize){
        size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < infoSize) {
                ...
                if(info.attackType == DPolyRInfo::AttackType::BITFLIP_RANGE_ABSTRACT){
                        ...
                        slope_point<T> lb_line, ub_line;
                        sigmoid_weighted_abstract<T>(rangeMin, rangeMax, input.low, input.high, &lb_line, &ub_line);
...
template<typename T>
void Sigmoid::applyInfo(Vector<T>& dest){
        if(infoPool.size() > 0){
                applyInfoSigmoid<T> <<<(infoPool.size() + 15) / 16, 16>>>(dest, activCst<T>(), activFac<T>(), nn.getConcreteBounds<T>(parent), infoPool.data(), infoPool.size());
        }
}

template<typename T>
void Sigmoid::eval(Vector<T>& dest, bool sound, bool precise) {
        ...
        evalSigmoidKernel<T> <<<(this->inputSize + 255) / 256, 256>>>(dest, activCst<T>(), activFac<T>(), nn.getConcreteBounds<T>(parent), this->inputSize);
	applyInfo(dest);
...
static __global__ void backSubstituteSigmoid(Tdest* destA, T* destb, const TA* exprA, const T* exprb, const Intv<T>* modelFac, const Intv<T>* modelCst, const size_t dest_N, const size_t expr_N, const size_t outputSize, const size_t inputSize,const DPolyRInfo *infopool, const size_t infosize)
{
...
static __global__ void backSubstituteSigmoidInit(
...
template<typename T>
void Sigmoid::backSubstitute(typename AffineExpr<T>::Queue& queue, const AffineExpr<T>& expr) const {
	...
	if (expr.A)
	{
		...
		if (expr.sound)
		{
			...
			if (expr.A->interval())
				if (expr.up)
					backSubstituteSigmoid<8, Intv<T>, Intv<T>, true,T> << <grid, block >> > (*A, *b, (const Intv<T>*)*expr.A, expr.b ? (const T*)*expr.b : nullptr, activFac<T>(), activCst<T>(), A->pitch(), expr.A->pitch(), this->outputSize, this->inputSize, infoPool.data(), infoPool.size());
				else
					backSubstituteSigmoid<8, Intv<T>, Intv<T>, false,T> << <grid, block >> > (*A, *b, (const Intv<T>*)*expr.A, expr.b ? (const T*)*expr.b : nullptr, activFac<T>(), activCst<T>(), A->pitch(), expr.A->pitch(), this->outputSize, this->inputSize, infoPool.data(), infoPool.size());
			else
				if (expr.up)
					backSubstituteSigmoid<8, T, Intv<T>, true,T> << <grid, block >> > (*A, *b, (const T*)*expr.A, expr.b ? (const T*)*expr.b : nullptr, activFac<T>(), activCst<T>(), A->pitch(), expr.A->pitch(), this->outputSize, this->inputSize,infoPool.data(),infoPool.size());
				else
					backSubstituteSigmoid<8, T, Intv<T>, false,T> << <grid, block >> > (*A, *b, (const T*)*expr.A, expr.b ? (const T*)*expr.b : nullptr, activFac<T>(), activCst<T>(), A->pitch(), expr.A->pitch(), this->outputSize, this->inputSize,infoPool.data(),infoPool.size());
		}
		else
		{
			...
			if (expr.up)
				backSubstituteSigmoid<8, T, T, true,T> << <grid, block >> > (*A, *b, (const T*)*expr.A, expr.b ? (const T*)*expr.b : nullptr, activFac<T>(), activCst<T>(), A->pitch(), expr.A->pitch(), this->outputSize, this->inputSize,infoPool.data(),infoPool.size());
			else
				backSubstituteSigmoid<8, T, T, false,T> << <grid, block >> > (*A, *b, (const T*)*expr.A, expr.b ? (const T*)*expr.b : nullptr, activFac<T>(), activCst<T>(), A->pitch(), expr.A->pitch(), this->outputSize, this->inputSize,infoPool.data(),infoPool.size());
	...
	else
	{
		...
		if (expr.up)
			backSubstituteSigmoidInit<8, true,T> << <grid, block >> > (
				*A, A->pitch(), this->outputSize,
				expr.rows,
				activFac<T>());
		else
			backSubstituteSigmoidInit<8, false,T> << <grid, block >> > (
				*A, A->pitch(), this->outputSize,
				expr.rows,
				activFac<T>());
		// std::cout << "Enter Else\n";
