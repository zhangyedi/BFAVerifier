...
class Tanh: public NeuralNetwork::Layer {
...
public:
        Tanh(NeuralNetwork& nn, int size, const int parent) :NeuralNetwork::Layer(nn, size), activCstD(size, true), activCstS(size,true), activFacS(size, true), activFacD(size, true), parent(parent), inputSize(size) {}
        Tanh(NeuralNetwork& nn, int inputSize, int outputSize, const int parent) :NeuralNetwork::Layer(nn, outputSize), activCstD(outputSize, true), activCstS(outputSize,true), activFacS(outputSize, true), activFacD(outputSize, true), parent(parent), inputSize(inputSize) {}
        ...
        int addInfo(const DPolyRInfo& info) { 
                if (info.rangeMin < 0 && info.rangeMax > 0) {
                        std::cout << "Error, Tanh layer only supports non-crossing zero wl and wu" << std::endl;
                        info.print();
                        return -1;
                }
