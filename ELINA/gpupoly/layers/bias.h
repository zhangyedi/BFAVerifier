...
class Bias :public NeuralNetwork::Layer
{
	...
	std::shared_ptr<Vector<T>> b; //!< Constant Vector to be added
public:
	//! Constructor
	/*!
	...
	*/
	Bias(NeuralNetwork& nn,std::shared_ptr<Vector<T>> b, const int parent) :NeuralNetwork::Layer(nn, b->size()), b(b), parent(parent) {}
	Bias(NeuralNetwork& nn,const std::vector<Intv<T>>&b, const int parent) :NeuralNetwork::Layer(nn, b.size()), b(std::make_shared<Vector<T>>((b))), parent(parent) {}
	virtual void eval(Vector<double>& dest, bool sound, bool precise)
	{
		// if(precise){
		// 	nn.reEvaluateLayer(parent, AlwaysKeep<double>(), true, sound);
		// 	nn.reEvaluateLayer(parent, AlwaysKeep<double>(), false, sound);
		// }
	...
	{
		// if(precise){
		// 	nn.reEvaluateLayer(parent, AlwaysKeep<float>(), true, sound);
		// 	nn.reEvaluateLayer(parent, AlwaysKeep<float>(), false, sound);
		// }
	...
	void changeBiasSingle(int i, T rangeMin, T rangeMax)
	{
		b -> changeSingle(i, rangeMin, rangeMax);
	}
