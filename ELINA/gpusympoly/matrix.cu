...
template<typename T>
__global__ void modify_single_d(T* A, const size_t pitch, const size_t rowNum, const size_t colNum, T value)
{
	A[rowNum * pitch + colNum] = value;
}

template<typename T>
__global__ void modify_column_d(T* A, const size_t pitch, const size_t colNum, size_t m, T value)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx < m)
		A[idx * pitch + colNum] = value;
}

template<typename T>
__global__ void modify_column_range_d(Intv<T>* A, const size_t pitch, const size_t colNum, size_t m, T lb, T ub)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	Intv<T> value(lb, ub);
	if (idx < m)
...
template<typename T>
__global__ void modify_single_range_d(Intv<T>* A, const size_t pitch, const size_t rowNum, const size_t colNum, T lb, T ub)
{
	Intv<T> value(lb, ub);
	A[rowNum * pitch + colNum] = value;
}
...
template<typename T>
void Matrix<T>::modify_single(size_t rowNum, size_t colNum, T value){
	assert(rowNum < m_);
	assert(colNum < n_);
	assert(!interval());
	if(interval())
		modify_single_range_d<T> << <1, 1 >> > (*this, pitch(), rowNum, colNum, value, value);
	else
		modify_single_d<T> << <1, 1 >> > (*this, pitch(), rowNum, colNum, value);
}

template<typename T>
void Matrix<T>::modify_column(size_t colNum, T value){
	assert(colNum < n_);
	assert(!interval());
	if(interval())
		modify_column_range_d<T> << <(m_ + 1023) / 1024, 1024 >> > (*this, pitch(), colNum, m_, value, value);
	else
		modify_column_d<T> << <(m_ + 1023) / 1024, 1024 >> > (data_, pitch(), colNum, m_, value);
}

template<typename T>
void Matrix<T>::modify_single_range(size_t rowNum, size_t colNum, T lb, T ub){
	assert(rowNum < m_);
	assert(colNum < n_);
	assert(interval());
	modify_single_range_d<T> << <1, 1 >> > (*this, pitch(), rowNum, colNum, lb, ub);
}
...
template<typename T>
void Matrix<T>::convertToIntv(){
	if(interval())
		return;
	Matrix<T> res(m(), n(), true);
	dim3 block(1024, 1, 1);
	dim3 grid(m(), (n_ + 1023) / 1024, 1);
	backSubstituteDense<10, Intv<T>, T> << <grid, block >> > (res, res.pitch(), nullptr, n_, *this, pitch());
	gpuChkKer();
	*this = std::move(res);
}
