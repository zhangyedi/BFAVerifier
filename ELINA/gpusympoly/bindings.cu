...
int addReLUInfo(NeuralNetwork* nn, int layerIndex,const DPolyRInfo* info)
{
...
int clearReLUInfo(NeuralNetwork* nn, int layerIndex)
{
	ReLU *relulayer = dynamic_cast<ReLU*>((*nn)[layerIndex]);
	if(relulayer != nullptr){
		relulayer->clearInfo();
...
int getReLUInfoSize(NeuralNetwork* nn, int layerIndex)
{
	ReLU *relulayer = dynamic_cast<ReLU*>((*nn)[layerIndex]);
	if(relulayer != nullptr){
