...
def get_debug_setting_set():
    return set(os.getenv("BFA_MILP_DEBUG","").lower().split(','))
...
class QNNEncoding_MILP:
    def __init__(self, deepModel, W_dict, Delta_LL, args, all_lb_LL, all_ub_LL, verbose=None):
        ...
        activation_dict_mapping = {
            "relu": 0,
            "sigmoid": 1,
            "tanh": 2,
        }
        if "activation_type" not in args:
            print("No activation type is specified, default to ReLU (MILP encoding)")
            self.relu_N = 0
        else:
            self.relu_N = activation_dict_mapping[args.activation_type]
    ...
    def encode(self):
        ...
        for i, l in enumerate(self.encoded_dense_layer):
            ...
            if "encode_only_first_dense" in get_debug_setting_set():
                print("Warning: Only encoding the first dense layer for debugging purposes")
                break
    ...
    def encode_dense(self, in_layer, out_layer, layer_index, w_fp, b_fp):
        ...
        for out_index in range(out_layer.layer_size):
            ...
            if "skip_activation" in get_debug_setting_set():
                print("Warning: Skipping activation function for debugging purposes")
                continue
            
            if "force_relu_activation" in get_debug_setting_set():
                print("Warning: Forcing ReLU activation function for debugging purposes")
                self.gp_model.addGenConstrMax(out_layer.gp_DNN_vars_pos[out_index],
                                              [out_layer.gp_DNN_vars[out_index], 0])
                continue
            
            if "force_tanh_activation" in get_debug_setting_set():
                neg_vars = self.gp_model.addVar(lb=-np.inf)
                exp_x = self.gp_model.addVar(lb=0)
                exp_neg_x = self.gp_model.addVar(lb=0)
                exp_x_minus_neg_x = self.gp_model.addVar(lb=-np.inf)
                exp_x_plus_neg_x = self.gp_model.addVar(lb=-np.inf)

                self.gp_model.addConstr(neg_vars == -out_layer.gp_DNN_vars[out_index])
                self.gp_model.addGenConstrExp(out_layer.gp_DNN_vars[out_index], exp_x)
                self.gp_model.addGenConstrExp(neg_vars, exp_neg_x)
                self.gp_model.addConstr(exp_x_minus_neg_x == exp_x - exp_neg_x)
                self.gp_model.addConstr(exp_x_plus_neg_x == exp_x + exp_neg_x)
                self.gp_model.addConstr(out_layer.gp_DNN_vars_pos[out_index] * exp_x_plus_neg_x == exp_x_minus_neg_x)
                self.gp_model.update()
                
                continue
            
            # encode ReLU function
            # Newly-Added-0202: ReLU
            if not out_layer.if_last and self.relu_N == 0:  # hidden_layer's ReLU function
                self.gp_model.addGenConstrMax(out_layer.gp_DNN_vars_pos[out_index],
                                              [out_layer.gp_DNN_vars[out_index], 0])
                self.gp_model.update()

            # encode Sigmoid function:
            # Newly-Added-0202: Sigmoid
            if not out_layer.if_last and self.relu_N == 1:  # hidden_layer's Sigmoid function
                vars_Negative = self.gp_model.addVar(lb=-np.inf)
                self.gp_model.addConstr(vars_Negative == -out_layer.gp_DNN_vars[out_index])
                exp_vars_neg = self.gp_model.addVar(lb=0)
                self.gp_model.addGenConstrExp(vars_Negative, exp_vars_neg)
                exp_vars_neg_plus_one = self.gp_model.addVar()
                self.gp_model.addConstr(exp_vars_neg_plus_one == 1 + exp_vars_neg)
                self.gp_model.addConstr(out_layer.gp_DNN_vars_pos[out_index] * exp_vars_neg_plus_one == 1)

                self.gp_model.update()

            # encode tanh function:
            # Newly-Added-0202: tanh
            if not out_layer.if_last and self.relu_N == 2:  # hidden_layer's Tanh function
                neg_vars = self.gp_model.addVar(lb=-np.inf)
                exp_x = self.gp_model.addVar(lb=0)
                exp_neg_x = self.gp_model.addVar(lb=0)
                exp_x_minus_neg_x = self.gp_model.addVar(lb=-np.inf)
    ...
    def sat(self, args):
        ...
        if "output_iis_gurobi" in get_debug_setting_set():
            if self.gp_model.Status == GRB.INFEASIBLE:
                self.gp_model.computeIIS()

            self.gp_model.write('my_iis.ilp') 
    ...
    def output_not_argmax_gurobi(self, max_index):
        ...
        for i in range(len(self.output_gp_vars)):
            ...
            self.gp_model.addConstr(
                self.output_gp_vars[i] <= self.output_gp_vars[int(max_index)] + bigM * k_i - 0.0001)
    ...
    def get_all_neuron_values(self):
        all_neuron_values = []
        all_neuron_values_pos = []

        for i, l in enumerate(self.encoded_dense_layer):
            all_neuron_values.append(
                np.array(
                    [var.X for var in l.gp_DNN_vars]
                )
            )
            if i != len(self.encoded_dense_layer) - 1:
                all_neuron_values_pos.append(
                    np.array(
                        [var.X for var in l.gp_DNN_vars_pos]
                    )
                )

        return all_neuron_values, all_neuron_values_pos
    ...
    def __init__(self, layer_size, gp_model, qu_bit, lb_LL=None, ub_LL=None, signed_output=False, if_last=False,
        ...
        if signed_output:
            ...
            if self.if_input:
                ...
                # hidden layer: add vars before activation function
                self.gp_DNN_vars = [
                    gp_model.addVar(lb=lb_LL[i], ub=ub_LL[i], vtype=GRB.CONTINUOUS) for i in range(layer_size)
                ]
                if "force_tanh_activation" in get_debug_setting_set():
                    self.gp_DNN_vars_pos = [
                        gp_model.addVar(lb=-1, ub=1, vtype=GRB.CONTINUOUS) for i in range(layer_size)
                    ]
                    gp_model.update()
                    return
                ...
                if "force_relu_activation" in get_debug_setting_set():
                    self.gp_DNN_vars_pos = [
                        gp_model.addVar(lb=0, ub=1, vtype=GRB.CONTINUOUS) for i in range(layer_size)
                    ]
                    gp_model.update()
                    return
                ...
                if self.relu_N == 0:
                    self.gp_DNN_vars_pos = [
                        gp_model.addVar(ub=max(ub_LL[i], 0), vtype=GRB.CONTINUOUS) for i in range(layer_size)
                    ]
                elif self.relu_N == 1:  # Newly-Added-0202: Sigmoid
                    self.gp_DNN_vars_pos = [
                        gp_model.addVar(ub=1, vtype=GRB.CONTINUOUS) for i in range(layer_size)
                    ]
                else:  # Newly-Added-0202: tanh
                    self.gp_DNN_vars_pos = [
                        gp_model.addVar(lb=-1, ub=1, vtype=GRB.CONTINUOUS) for i in range(layer_size)
                    ]
        gp_model.update()
