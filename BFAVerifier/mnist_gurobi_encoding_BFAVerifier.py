        ...
        if signed_output:
            ...
            if self.if_input:
                self.gp_DNN_vars_pos = [
                    gp_model.addVar(ub=1, vtype=GRB.CONTINUOUS) for i in range(layer_size)  # [-100,100]
                ]
            else:
...
def check_robustness_gurobi(milp_encoding, x, args, prediction):
    x = x.flatten()
    ...
    for out_index in range(len(milp_encoding.deepPolyNets_DNN.layers[-1].neurons)):
        lb = milp_encoding.deepPolyNets_DNN.layers[-1].neurons[out_index].concrete_lower_noClip
        ub = milp_encoding.deepPolyNets_DNN.layers[-1].neurons[out_index].concrete_upper_noClip
        print("The output bound for the output neuron", out_index, "is: [", lb, ',', ub, ']')
        if out_index != prediction:
            otherMax = max(otherMax, ub)
        else:
            target_lb = lb

    if target_lb > otherMax:
